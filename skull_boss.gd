...
func _on_shootingdelay_timeout() -> void:
	...
	var rand = randi() % 4
	if rand == 0:
		projectile.get_node("RigidBody2D/AnimatedSprite2D").animation = "bone1"
	elif rand == 1:
		projectile.get_node("RigidBody2D/AnimatedSprite2D").animation = "bone2"
	elif rand == 2:
		projectile.get_node("RigidBody2D/AnimatedSprite2D").animation = "bone3"
	else:
		projectile.get_node("RigidBody2D/AnimatedSprite2D").animation = "tooth"
